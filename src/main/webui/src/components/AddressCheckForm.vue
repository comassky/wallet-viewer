...
<template>
  <form @submit.prevent="verify" :aria-busy="checking">
    ...
    <div class="mt-3 min-h-10">
      <p v-if="error" role="alert" class="flex items-start gap-2 text-xs text-rose-400"><UiIcon name="alert" class="mt-0.5 shrink-0" /><span>{{ error }}</span></p>
      <p v-else-if="result?.belongs" role="status" class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-emerald-400">
      ...
      <p v-else-if="result" role="status" class="flex items-start gap-2 text-xs text-amber-400"><UiIcon name="alert" class="mt-0.5 shrink-0" /><span>Not found in the first {{ result.checked }} receive and change addresses checked. Addresses beyond this range have not been checked.</span></p>
    </div>
  </form>
</template>