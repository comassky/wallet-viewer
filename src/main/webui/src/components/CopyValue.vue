...
<template>
  <span class="min-w-0 max-w-full align-middle" :class="statusRight ? 'flex items-center gap-2' : 'inline-block'">
    ...
    >{{ display ?? value }} <UiIcon :name="copied ? 'check' : 'copy'" class="ml-1 inline-block h-3 w-3 align-middle transition-colors" :class="copied ? 'text-emerald-400' : 'text-slate-500 group-hover/copy:text-accent group-focus-visible/copy:text-accent'" /></button>
    </AppTooltip>
