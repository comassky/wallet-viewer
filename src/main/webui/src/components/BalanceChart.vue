...
import { createChart, AreaSeries, ColorType, CrosshairMode, LineStyle, type IChartApi, type ISeriesApi, type UTCTimestamp } from 'lightweight-charts';
...
// Cumulative balance; pending transactions (no timestamp) plot at "now", times kept strictly ascending.
function buildData() {
  const ordered = [...props.transactions].sort((a, b) => (a.timestamp ?? Infinity) - (b.timestamp ?? Infinity) || a.height - b.height);
  ...
  return ordered.map(tx => {
    running += tx.amount;
    let time = tx.timestamp ?? now;
    if (time <= last) time = last + 1;
    last = time;
    return { time: time as UTCTimestamp, value: running * factor };
  });
}
...
watch([() => props.transactions, () => props.currency], render);
