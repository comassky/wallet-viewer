...
class TransactionDetailsServiceTest {
    ...
    void normalizesIdAndComputesTrueFeeFromAllDistinctExternalParents() {
        ...
        tx.setVersion(2);
        tx.setLockTime(123);
        TransactionWitness stack = new TransactionWitness(1);
        stack.setPush(0, new byte[]{1, 2, 3});
        tx.getInput(0).setWitness(stack);
        authorize(tx);
        electrum.put(first);
        electrum.put(second);
    ...
    void rendersTaprootWitnessScriptHashAndUnsupportedOutputs() {
        ...
        for (int i = 0; i < tx.getOutputs().size(); i++) {
            ...
            assertEquals(Utils.HEX.encode(tx.getOutput(i).getScriptBytes()), dto.outputs().get(i).scriptHex());
        }
    }
